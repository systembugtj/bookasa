using System;
using System.IO;
using System.Collections.Generic;
using System.Collections.ObjectModel;
using Arcadia.Bookasa.Data;

namespace Arcadia.Bookasa.Data
{
    public class Books : Collection<Book>
    {
        Dictionary<string, FileSystemWatcher> watchers = new Dictionary<string, FileSystemWatcher>();

        public event EventHandler ItemsUpdated;

        protected override void ClearItems()
        {
            base.ClearItems();
            watchers.Clear();
        }

        protected override void InsertItem(int index, Book item)
        {
            base.InsertItem(index, item);
            if (!watchers.ContainsKey(item.Path))
            {
                FileSystemWatcher watcher = new FileSystemWatcher(item.Path, "*.jpg");
                watcher.EnableRaisingEvents = true;
                watcher.Created += new System.IO.FileSystemEventHandler(OnBookCreated);
                watcher.Deleted += new System.IO.FileSystemEventHandler(OnBookDeleted);
                watcher.Renamed += new System.IO.RenamedEventHandler(OnBookRenamed);
                watchers.Add(item.Path, watcher);
            }
        }

        void OnBookRenamed(object sender, System.IO.RenamedEventArgs e)
        {
            int index = -1;
            for (int i = 0; i < Items.Count; i++)
            {
                if (Items[i].FullPath == e.OldFullPath)
                {
                    index = i;
                    break;
                }
            }
            if (index >= 0)
                Items[index] = new Book(e.FullPath);

            if (ItemsUpdated != null)
                ItemsUpdated(this, new EventArgs());
        }

        void OnBookDeleted(object sender, System.IO.FileSystemEventArgs e)
        {
            int index = -1;
            for (int i = 0; i < Items.Count; i++)
            {
                if (Items[i].FullPath == e.FullPath)
                {
                    index = i;
                    break;
                }
            }
            if (index >= 0)
                Items.RemoveAt(index);

            if (ItemsUpdated != null)
                ItemsUpdated(this, new EventArgs());
        }

        void OnBookCreated(object sender, System.IO.FileSystemEventArgs e)
        {
            Items.Add(new Book(e.FullPath));

            if (ItemsUpdated != null)
                ItemsUpdated(this, new EventArgs());
        }
    }
}
