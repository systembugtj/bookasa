...
IF  EXISTS (SELECT * 
			FROM sys.objects 
			WHERE object_id = OBJECT_ID(N'[DublinCore].[Policy]') AND type in (N'U'))
DROP TABLE [DublinCore].[Policy]
GO
/*
 * A plan or course of action by an authority, intended to influence and determine decisions, 
 *	actions, and other matters.
 * See http://purl.org/dc/terms/Policy 
 */ 
CREATE TABLE [DublinCore].[Policy](
...
EXEC sys.sp_addextendedproperty 
	...
	@value=N'A plan or course of action by an authority, intended to influence and determine decisions, actions, and other matters.', 
	@level0type=N'SCHEMA',
...
EXEC sys.sp_addextendedproperty 
	@name=N'MS_Description', 
	@value=N'The unique Identifer for the Policy.' , 
	@level0type=N'SCHEMA',
	@level0name=N'DublinCore', 
	@level1type=N'TABLE',
	@level1name=N'Policy',
